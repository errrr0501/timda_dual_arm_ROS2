...
namespace slider_hardware
{
  static const std::string HW_NAME = "SliderHardware";
  ...
  void SliderHardware::connect(std::string side, std::string usb_port, int baud_rate)
  {
    ...
    if (!ct)
      {
          std::cout << "Connect " + usb_port + " fail!!!" << std::endl;
          RCLCPP_ERROR(rclcpp::get_logger(HW_NAME), "Could not connect to slider");
      }
  }

  void SliderHardware::write_commands()
  {
      goal_pos = (double)100000.0*(hw_commands_[joint_index_]+0.8);
      // goal_pos = -1*(double)100000.0*(msg->pos);
      if(goal_pos > 80000.0) goal_pos = 80000;
      if(goal_pos < 0.0) goal_pos = 0;
  ...
  */
  void SliderHardware::update_hardware_info()
  {
...
hardware_interface::CallbackReturn SliderHardware::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger(HW_NAME), "connect_port: %s", (usb_port + side).c_str());
  RCLCPP_INFO(rclcpp::get_logger(HW_NAME), "baud_rate: %d", baud_rate);

  connect(side, usb_port, baud_rate);
...
hardware_interface::CallbackReturn SliderHardware::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
...
hardware_interface::CallbackReturn SliderHardware::on_deactivate(const rclcpp_lifecycle::State & /*previous_state*/)
{
...
hardware_interface::return_type SliderHardware::read(const rclcpp::Time & /* time */, const rclcpp::Duration & /* period */)
{
...
hardware_interface::return_type SliderHardware::write(const rclcpp::Time & /* time */, const rclcpp::Duration & /* period */)
{
